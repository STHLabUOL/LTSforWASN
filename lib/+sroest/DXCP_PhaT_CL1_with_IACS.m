function [SROest_ppm, SROrem_ppm_vec, SROrem_ppm_raw_vec, ATDres_smp_vec, SSOest_smp, GCCF1, GCCF2, GCCF2_avg,...
    ...
    RTFs, IACS, preIACS, preIACS_unrestr] = DXCP_PhaT_CL1_with_IACS(sig_z, parDXCPPhaT_cl, OnlineIACS)
...
% :return sig_z2_res:                    Resampled signal chanel 2 (vector)
    ...
    preIACS= zeros(1,FrNum_L);
    preIACS_unrestr = zeros(1,FrNum_L);
...
%             GCSD2_act = DXCPPhaT_Cont_GCSD_avg(:,end).*conj(DXCPPhaT_Cont_GCSD_avg(:,1));
        ...
        OnlineIACS.process(GCSD2_avg);
        IACS(ell) = OnlineIACS.ACS;
        preIACS(ell) = OnlineIACS.preACS;
        preIACS_unrestr(ell) = OnlineIACS.preACS_unrestr;
